...
## Running unit tests
Run the unit tests by running `npm run test` command. After code changes, make sure no failures are present. Refer to @https://angular.dev/guide/testing for guidelines on how to test with vitest.
If you wish to run a single suite, use the `--include` flag, e.g. `npm run test -- --include=src/app/app.spec.ts`, or use a glob pattern.
...
## Coverage
Make sure the coverage is 100% after adding any new code. Run the coverage using `npm run test -- --coverage` command.
